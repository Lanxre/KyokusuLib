...
export function useSecuritySettings() {
    ...
    const validate = (): boolean => {
        ...
        if (form.email && form.email !== user?.email) {
            ...
            if (!form.currentPassword) {
                errors.value.confirmEmailPassword = "Введите пароль для подтверждения смены email.";
                isValid = false;
            }
        }
        ...
        if (form.newPassword || form.repeatPassword) {
            ...
            if (!passwordRegex.test(form.newPassword)) {
                errors.value.newPassword = "Пароль: 6-50 симв., латиница, 1 заглавная, 1 строчная.";
                isValid = false;
            }
    ...
    const handleSendResetLink = async () => {
        ...
        try {
            ...
            resetSuccessMessage.value = "Ссылка для сброса пароля отправлена на вашу почту.";
    ...
    const saveChanges = async () => {
        ...
        try {
            ...
            if ((form.currentPassword !== "" && form.currentPassword !== null && form.currentPassword !== undefined)
               && (form.newPassword !== "" && form.newPassword !== null && form.newPassword !== undefined)
            ) {
                const { error, response } = await useApi("/api/profile/password", {
					credentials: "include",
				})
                .post({ old_password: form.currentPassword, new_password: form.newPassword })
                .json();
            ...
            if (form.email !== "" && form.email !== null && form.email !== undefined) {
                const { error, response } = await useApi("/api/profile/email", {
					credentials: "include",
				})
                .post({ old_password: form.currentPassword, email: form.email })
                .json();
