...
export function useTeam() {
  ...
  const fetchTeam = async (slug: string) => {
    ...
    error.value = null
    try {
      const { data, error: fetchError } = await useApi<Team>(`/api/teams/${slug}`)
  ...
  const createTeam = async (form: { name: string; slug: string; description: string }) => {
    isLoading.value = true
    try {
  ...
  const updateTeam = async (slug: string, form: FormData) => {
    isLoading.value = true
    try {
      const { data, error: updateError } = await useApi<Team>(`/api/teams/${slug}`, {
        method: 'PATCH',
        body: form
      })

      if (updateError.value) throw new Error(updateError.value.data?.error || 'Failed to update team')
      
      if (team.value && data.value) {
        Object.assign(team.value, data.value)
      }
      
      notify({ type: 'success', title: 'Успешно', content: 'Команда обновлена' })
      return data.value
    } catch (e: any) {
      notify({ type: 'error', title: 'Ошибка', content: e.message })
      return null
  ...
  const joinTeam = async (slug: string) => {
    try {
      const { error: joinError } = await useApi(`/api/teams/${slug}/join`, { method: 'POST' })
      
      if (joinError.value) throw new Error(joinError.value.data?.error)
      
      notify({ type: 'success', title: 'Успешно', content: 'Вы вступили в команду' })
      return true
    } catch (e: any) {
      notify({ type: 'error', title: 'Ошибка', content: e.message })
      return false
