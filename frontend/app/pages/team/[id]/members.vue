...
const loadMembers = async (isLoadMore = false) => {
    ...
    try {
        const newMembers = await getTeamMembers(slug.value, limit, offset.value);
...
useSeoMeta({
    title: () => team.value?.name ? `Участники: ${team.value.name} - KyokusuLib` : 'Участники команды',
});
</script>
...
<template>
    <div class="min-h-screen bg-zinc-50 dark:bg-[#0f0f0f] text-zinc-900 dark:text-zinc-200 flex flex-col font-sans transition-colors duration-300">
        <div class="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            ...
            <div v-if="isLoading && members.length === 0" class="flex flex-col gap-4">
                <div v-for="i in 10" :key="i" class="h-20 rounded-2xl bg-zinc-100 dark:bg-zinc-800/50 animate-pulse w-full"></div>
            </div>
            
            <div v-else-if="members.length > 0" class="flex flex-col gap-4">
                <TeamMemberCard 
