...
<template>
	<div class="min-h-screen bg-zinc-50 dark:bg-[#0f0f0f] transition-colors duration-300">
		<div class="max-w-5xl mx-auto px-4 py-8 space-y-8">
			<div>
				<h1 class="text-3xl font-black text-zinc-900 dark:text-white">Панель модерации</h1>
				<p class="text-zinc-500 dark:text-zinc-400 mt-2">
					Ожидающие подтверждения тома и главы, добавленные пользователями.
				</p>
			</div>

			<div v-if="!hasPermission(KyokusuAppRole.MODERATOR)" class="p-8 text-center text-red-500 font-bold bg-red-50 dark:bg-red-500/10 rounded-2xl">
				У вас нет доступа к этому разделу.
			</div>

			<div v-else-if="isLoading" class="py-24 flex flex-col items-center justify-center text-zinc-400">
			...
			<div v-else-if="pendingContent">
				<div v-if="pendingContent.volumes.length === 0 && pendingContent.chapters.length === 0" class="py-24 flex flex-col items-center justify-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-[3rem] text-zinc-400 bg-zinc-50/50 dark:bg-zinc-900/10">
				...
				<div v-else class="grid grid-cols-1 md:grid-cols-2 gap-8">
					<!-- Volumes Queue -->
					<div v-if="pendingContent.volumes.length > 0" class="space-y-4">
					...
					<!-- Chapters Queue -->
					<div v-if="pendingContent.chapters.length > 0" class="space-y-4">
						<div class="flex items-center gap-2 mb-6">
