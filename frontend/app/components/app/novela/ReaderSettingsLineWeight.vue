...
const { lineWeight } = useReaderSettings();

const fillPercent = computed(() => ((lineWeight.value - 1.0) / (2.5 - 1.0)) * 100);
		...
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-2.5">
				<div class="w-8 h-8 rounded-lg bg-yellow-500/10 flex items-center justify-center">
					<Icon name="ph:text-align-center-bold" size="16" class="text-yellow-600 dark:text-yellow-400" />
				</div>
				<label class="text-sm font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Межстрочный интервал</label>
			</div>
			<div class="flex items-baseline gap-0.5">
				<span class="text-lg font-black font-mono text-yellow-500 tabular-nums">{{ formatWeight(lineWeight) }}</span>
			</div>
		</div>
